...
(async () => {
  const name = "Aflac";
  const careerPageUrl = "https://careers.aflac.com/go/View-All-Jobs/8697400/";
  const slug = "aflac";
  ...
  if (existing) {
    ...
    const [newCo] = await sql`
      ...
      VALUES (${name}, ${careerPageUrl}, 'pending', 'sap_sf', ${finalSlug}, ${token()}, 29)
  ...
  const filename = `aflac-standalone.csv`;
