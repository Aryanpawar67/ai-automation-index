...
(async () => {
  const name = "AmTrust Financial Services, Inc.";
  const careerPageUrl = "https://careers-amtrustgroup.icims.com/jobs/search?ss=1";
  const slug = "amtrust-financial-services";
  ...
  if (existing) {
    ...
    const [newCo] = await sql`
      ...
      VALUES (${name}, ${careerPageUrl}, 'pending', 'icims', ${finalSlug}, ${token()}, 148)
  ...
  const filename = `amtrust-financial-standalone.csv`;
