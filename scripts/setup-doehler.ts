...
(async () => {
  const name = "Doehler India Private Limited";
  const careerPageUrl = "https://jobs.doehler.com/search/?q=&locationsearch=&searchResultView=LIST&pageNumber=0";
  const slug = slugify(name); // doehler-india-private-limited

  // Check for an existing row first (idempotent)
  ...
  if (existing) {
    ...
    const [newCo] = await sql`
      ...
      VALUES (${name}, ${careerPageUrl}, 'pending', NULL, ${finalSlug}, ${token()})
  ...
  const filename = `doehler-india-standalone.csv`;
