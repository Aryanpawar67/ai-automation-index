...
(async () => {
  const name = "Amica Insurance";
  const careerPageUrl = "https://careers.amica.com/search/jobs";

  // Check for an existing row first (idempotent)
  const [existing] = await sql`SELECT id, slug FROM companies WHERE slug = 'amica-insurance'`;
  ...
  if (existing) {
    ...
    const [newCo] = await sql`
      ...
      VALUES (${name}, ${careerPageUrl}, 'pending', NULL, ${finalSlug}, ${token()}, 19)
  ...
  const filename = `amica-insurance-standalone.csv`;
