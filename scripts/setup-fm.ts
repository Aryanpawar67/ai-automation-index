...
(async () => {
  const name = "FM";
  const careerPageUrl = "https://careers.fm.com/careers-home/jobs";
  const slug = "fm";
  ...
  if (existing) {
    ...
    const [newCo] = await sql`
      ...
      VALUES (${name}, ${careerPageUrl}, 'pending', NULL, ${finalSlug}, ${token()}, 61)
  ...
  const [batch] = await sql`
    ...
    VALUES ('fm-standalone.csv', ${name}, 'admin', 'scraping', 1)
    RETURNING id
  `;
  await sql`
