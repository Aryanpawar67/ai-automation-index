...
(async () => {
  const name = "Honeywell Aerospace";
  const careerPageUrl = "https://icfcjb.fa.ocs.oraclecloud.com/hcmUI/CandidateExperience/en/sites/Aerospace/jobs?mode=location";
  const slug = slugify(name); // honeywell-aerospace

  const [existing] = await sql`SELECT id, slug FROM companies WHERE slug = ${slug}`;
  ...
  if (existing) {
    ...
    await sql`
      UPDATE companies SET
        ...
        career_page_url = ${careerPageUrl}, ats_type = 'oracle_hcm'
      WHERE id = ${companyId}
    `;
  } else {
    const base = slug;
    ...
    const [newCo] = await sql`
      ...
      VALUES (${name}, ${careerPageUrl}, 'pending', 'oracle_hcm', ${finalSlug}, ${token()})
  ...
  const filename = `honeywell-aerospace-standalone.csv`;
