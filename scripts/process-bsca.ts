/**
 * Set up Blue Shield of California (Oracle HCM, 99 roles), reset state,
 * create batch, fire scrape, poll until complete, then trigger analyse.
 * Run: npx tsx --env-file=.env.local scripts/process-bsca.ts
 */
import { neon }       from "@neondatabase/serverless";
import { randomBytes } from "crypto";

const sql       = neon(process.env.DATABASE_URL!);
const NAME      = "Blue Shield of California";
const SLUG_BASE = "blue-shield-of-california";
const URL_      = "https://ecge.fa.us2.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX_1003/jobs";
const TOTAL     = 99;
...
(async () => {
  ...
  if (existing) {
    ...
    const [newCo] = await sql`
      ...
      VALUES (${NAME}, ${URL_}, 'pending', 'oracle_hcm', ${finalSlug}, ${token()}, ${TOTAL})
      RETURNING id, slug
    `;
  ...
  const [batch] = await sql`
    ...
    VALUES ('bsca-standalone.csv', ${NAME}, 'admin', 'scraping', 1)
    RETURNING id
  `;
  await sql`
