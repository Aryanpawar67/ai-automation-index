...
(async () => {
  const name = "Assurant";
  const careerPageUrl = "https://jobs.assurant.com/en/jobs/?search=&location=";
  const slug = "assurant";
  ...
  if (existing) {
    ...
    const [newCo] = await sql`
      ...
      VALUES (${name}, ${careerPageUrl}, 'pending', NULL, ${finalSlug}, ${token()}, 147)
  ...
  const filename = `assurant-standalone.csv`;
