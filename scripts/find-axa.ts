...
async function main() {
  const name = 'AXA US';
  const careerPageUrl = 'https://careers.axa.com/careers-home/jobs';
  const atsType = null;
  ...
  const [newCo] = await sql`
    ...
    VALUES (${name}, ${careerPageUrl}, 'pending', ${atsType}, ${finalSlug}, ${token()})
    RETURNING id, name, slug
  `;
  console.log(`Created company: "${newCo.name}" slug="${newCo.slug}" id=${newCo.id}`);
  ...
  await sql`
    ...
    VALUES (${batch.id}, ${newCo.id}, ${name}, '', '')
  `;
  console.log(`Created POC stub`);
  ...
  console.log(`\nBatch admin: http://localhost:3003/admin/batches/${batch.id}`);
  console.log(`Monitor:     http://localhost:8288`);
}
